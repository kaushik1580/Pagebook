<template>
     <div class="post" style="float:left">
        ...
        <v-card class="" max-width="800" style="margin:0px 0px 20px 0px" outlined v-for="(post,index) in displayPosts" :key="post.userId">
        <v-img @click="toPost(post.postDTO.postId)" height="300" :src="post.postDTO.postUrl" float="right"></v-img>
        ...
        <v-chip class="ma-1" v-for="value in post.reactionsDTOS" :key="value.reactionType" @click="reactTo(value.reactionType, post.postDTO.postId)">{{reaction_data[value.reactionType]}} {{value.count}}</v-chip>
...
export default {
    ...
    mounted() {
        ...
        Vue.axios.get(`http://10.177.1.69:8081/pb/post/getUserPosts/${uid}`)
            .then((resp) => {
        ...
        //     console.log(resp.data)
        // })
        // console.log(this.userData.type === 2 && !this.userData.isFriend)
    },
    methods:{
        reactTo(val, id){
            console.log(val + "   " + id)
